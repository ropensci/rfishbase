...
XML files for `r length(fish.data)` (as accessed on 14 May, 2012) of
...
knitcitations::functiontable("rfishbase", caption='A list of each of the functions and data objects provided by rfishbase')
````
...
``` {r relabel, include=FALSE, echo=FALSE }
# Relabel marine=true/false to Marine, Non-marine
i <- dat[["reef"]] 
dat$reef[i] <- "Reef"
dat$reef[!i] <- "Non-reef"
````
...
``` {r Figure1, fig.height=4, fig.width=6, fig.cap='Fraction of marine species in the eight largest orders of teleost fishes', dependson="libraries"}
...
``` {r Figure2, fig.height=4, fig.width=6, fig.cap='Scatterplot maximum age with length observed in each species. Color indicates marine or freshwater species.', dependson="libraries"}
...
``` {r Figure3, fig.height=3, fig.width=4, fig.cap='Distribution of maximum age for reef-associated and non-reef associated fish', dependson="libraries"}
...
which shows a significant correlation between maximum age and standard length, p-value `r format.pval(2*pt(abs(coef(corr.model)[,'t value']["age"]), corr.model$df, lower.tail=F))`. 
...
number of matches.  This reveals that there are `r labrids.on.reefs[["TRUE"]]` 
labrid species associated with reefs, and `r gobies.on.reefs[["TRUE"]]` goby species
...
which returns a non-significant correlation (p = `r corr.summary$coefficients[,4]`). 

``` {r Figure4, fig.height=3, fig.width=6, fig.cap='Correcting for phylogeny, size is not correlated with maximum depth observed in  labrids', dependson="libraries"}
...
where the Brownian motion model has an AIC score of `r bm$aic` while
the OU model has a score of `r ou$aic`, suggesting that
`r names(which.min(list(BM=bm$aic,OU=ou$aic)))` is the better model.
...
``` {r echo=FALSE}
save(list=ls(), file="rfishbase.rda")
````
