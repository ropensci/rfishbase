...
XML files for `r length(fish.data)` of its species entries.
...
``` {r Figure1, fig.height=4, fig.width=6, fig.cap='Fraction of marine species in the eight largest orders of teleost fishes'}
...
``` {r Figure2, fig.height=4, fig.width=6, fig.cap='Scatterplot maximum age with maximum length observed in each species. Color indicates marine or freshwater species.'}
...
``` {r Figure3, fig.height=3, fig.width=3, fig.cap='Distribution of maximum age for reef-associated and non-reef associated fish'}
...
which shows a significant correlation between length and age (p = `r corr.model$coefficients[,4]`).  
...
``` {r Figure4, fig.height=3, fig.width=6, fig.cap='Correcting for phylogeny, maximum size is not correlated with maximum depth observed in  labrids'}
