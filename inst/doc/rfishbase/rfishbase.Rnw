...
Stuff about Machine access to data, role of large scale data in ecology \citep{Jones2006d}, \citep{Hanson2011a}, \citep{Reichman2011a}
...
species.names <- sapply(fish.data[myfish], `[[`, 'ScientificName')
@
...
Drop any taxa for which we were unable to find data from the tree
<<>>=
...
drop any character data that doesn't match a species on the tree
<<>>=
